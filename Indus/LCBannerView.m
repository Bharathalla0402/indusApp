...
- (void)addSubviewToScrollView:(UIScrollView *)scrollView {
    ...
    for (int i = 0; i < self.count + 2; i++) {
        ...
        NSString *strname=[[NSUserDefaults standardUserDefaults] objectForKey:@"banar"];
        
        if ([strname isEqualToString:@"b1"])
        {
        ...
        else
        {
            ...
            NSObject * object2 = [prefs objectForKey:@"bana"];
            if(object2 != nil)
            {
                ...
                if (self.imageName.length > 0) {    // from local
                 ...
                 //   image = [self squareImageFromImage:image
                 //                         scaledToSize:self.frame.size.height];
                ...
                imageView.clipsToBounds          = YES;
                imageView.userInteractionEnabled = YES;
                imageView.backgroundColor=[UIColor colorWithPatternImage:[UIImage imageNamed:@"black_strip1.png"]];
                // imageView.backgroundColor=[UIColor colorWithRed:255.0/255.0f green:174.0/255.0f blue:185.0/255.0f alpha:1.0];
...
- (UIImage *)squareImageFromImage:(UIImage *)imaged scaledToSize:(CGFloat)newSize {
    CGAffineTransform scaleTransform;
    CGPoint origin;
    
    if (imaged.size.width > imaged.size.height) {
        CGFloat scaleRatio = newSize / imaged.size.height;
        scaleTransform = CGAffineTransformMakeScale(scaleRatio, scaleRatio);
        
        origin = CGPointMake(-(imaged.size.width - imaged.size.height) / 2.0f, 0);
    } else {
        CGFloat scaleRatio = newSize / imaged.size.width;
        scaleTransform = CGAffineTransformMakeScale(scaleRatio, scaleRatio);
        
        origin = CGPointMake(0, -(imaged.size.height - imaged.size.width) / 2.0f);
    }
    ...
    [imaged drawAtPoint:origin];
    
    imaged = UIGraphicsGetImageFromCurrentImageContext();
...
- (void)setPageDistance:(CGFloat)pageDistance {
    ...
    if (pageDistance != LCPageDistance) {
        ...
        frame.origin.y         = self.frame.size.height - 1.0f - pageDistance;
