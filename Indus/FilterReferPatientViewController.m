//
//  FilterReferPatientViewController.m
//  Indus
//
//  Created by think360 on 07/06/17.
...
@interface FilterReferPatientViewController ()<ApiRequestdelegate,UITableViewDelegate,UITableViewDataSource,CKCalendarDelegate>
{
    ...
    UIView *footerview2;
    UILabel *loadLbl,*locationNamelabel,*ResultsLabel;
...
@implementation FilterReferPatientViewController
...
-(void)CustomView
{
    ...
    titlelab.text=@"Referred History";
  ...
  //  NotificationButton.userInteractionEnabled=NO;
    ...
    UIView *ReferedView=[[UIView alloc] initWithFrame:CGRectMake(0, topView.frame.size.height+topView.frame.origin.y, self.view.frame.size.width, 60)];
    ReferedView.backgroundColor=[UIColor colorWithRed:49.0/255.0f green:89.0/255.0f blue:168.0/255.0f alpha:1.0];
    [self.view addSubview:ReferedView];
    
   
    
    
    UIView *leftview=[[UIView alloc]initWithFrame:CGRectMake(0, 0, ReferedView.frame.size.width/2-20, ReferedView.frame.size.height)];
    leftview.backgroundColor=[UIColor clearColor];
    [ReferedView addSubview:leftview];
    
    UIImageView *leftDateimage=[[UIImageView alloc] initWithFrame:CGRectMake(10, leftview.frame.size.height/2-10, 20, 20)];
    leftDateimage.image=[UIImage imageNamed:@"pro1.png"];
    leftDateimage.contentMode = UIViewContentModeScaleAspectFit;
    [leftview addSubview:leftDateimage];
    
    UIImageView *leftDateeditimage=[[UIImageView alloc] initWithFrame:CGRectMake(leftview.frame.size.width-25, leftview.frame.size.height/2-10, 20, 20)];
    leftDateeditimage.image=[UIImage imageNamed:@"pro3.png"];
    leftDateeditimage.contentMode = UIViewContentModeScaleAspectFit;
    [leftview addSubview:leftDateeditimage];
    
    UILabel *leftlinelabel=[[UILabel alloc]initWithFrame:CGRectMake(10, leftDateimage.frame.size.height+leftDateimage.frame.origin.y+5, leftview.frame.size.width-20, 2)];
    leftlinelabel.backgroundColor=[UIColor whiteColor];
    [leftview addSubview:leftlinelabel];
    
    fromDatelabel=[[UILabel alloc]initWithFrame:CGRectMake(35, leftview.frame.size.height/2-10, leftview.frame.size.width-60, 20)];
    fromDatelabel.backgroundColor=[UIColor clearColor];
    fromDatelabel.text=strStartDate;
    fromDatelabel.textAlignment=NSTextAlignmentCenter;
    fromDatelabel.font=[UIFont systemFontOfSize:14];
    fromDatelabel.textColor=[UIColor whiteColor];
    [leftview addSubview:fromDatelabel];
    
    UIButton *leftDateButton=[[UIButton alloc] initWithFrame:CGRectMake(0, 0, leftview.frame.size.width, leftview.frame.size.height)];
    [leftDateButton addTarget:self action:@selector(leftDateButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
    leftDateButton.backgroundColor=[UIColor clearColor];
    [leftview addSubview:leftDateButton];
    
    
    
    UILabel *Orlabel=[[UILabel alloc]initWithFrame:CGRectMake(ReferedView.frame.size.width/2-20, ReferedView.frame.size.height/2-20, 40, 50)];
    linelabel.backgroundColor=[UIColor clearColor];
    Orlabel.text=@"To";
    Orlabel.textAlignment=NSTextAlignmentCenter;
    Orlabel.font=[UIFont fontWithName:@"OpenSans-Bold" size:18.0];
    Orlabel.textColor=[UIColor whiteColor];
    [ReferedView addSubview:Orlabel];
    
    
    UIView *Rightview=[[UIView alloc]initWithFrame:CGRectMake(ReferedView.frame.size.width/2+20, 0, ReferedView.frame.size.width/2-20, ReferedView.frame.size.height)];
    Rightview.backgroundColor=[UIColor clearColor];
    [ReferedView addSubview:Rightview];
    
    UIImageView *RightDateimage=[[UIImageView alloc] initWithFrame:CGRectMake(10, Rightview.frame.size.height/2-10, 20, 20)];
    RightDateimage.image=[UIImage imageNamed:@"pro1.png"];
    RightDateimage.contentMode = UIViewContentModeScaleAspectFit;
    [Rightview addSubview:RightDateimage];
    
    UIImageView *RightDateeditimage=[[UIImageView alloc] initWithFrame:CGRectMake(Rightview.frame.size.width-25, Rightview.frame.size.height/2-10, 20, 20)];
    RightDateeditimage.image=[UIImage imageNamed:@"pro3.png"];
    RightDateeditimage.contentMode = UIViewContentModeScaleAspectFit;
    [Rightview addSubview:RightDateeditimage];
    
    UILabel *Rightlinelabel=[[UILabel alloc]initWithFrame:CGRectMake(10, RightDateimage.frame.size.height+RightDateimage.frame.origin.y+5, leftview.frame.size.width-20, 2)];
    Rightlinelabel.backgroundColor=[UIColor whiteColor];
    [Rightview addSubview:Rightlinelabel];
    
    RightDatelabel=[[UILabel alloc]initWithFrame:CGRectMake(35, Rightview.frame.size.height/2-10, Rightview.frame.size.width-60, 20)];
    RightDatelabel.backgroundColor=[UIColor clearColor];
    RightDatelabel.text=strEndDate;
    RightDatelabel.textAlignment=NSTextAlignmentCenter;
    RightDatelabel.font=[UIFont systemFontOfSize:14];
    RightDatelabel.textColor=[UIColor whiteColor];
    [Rightview addSubview:RightDatelabel];
    
    UIButton *RightDateButton=[[UIButton alloc] initWithFrame:CGRectMake(0, 0, Rightview.frame.size.width, Rightview.frame.size.height)];
    [RightDateButton addTarget:self action:@selector(RightDateButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
    RightDateButton.backgroundColor=[UIColor clearColor];
    [Rightview addSubview:RightDateButton];
    
    
    
    
    UIView *filterView=[[UIView alloc] initWithFrame:CGRectMake(0, ReferedView.frame.size.height+ReferedView.frame.origin.y, self.view.frame.size.width, 50)];
    ...
    UIButton *button=[[UIButton alloc] initWithFrame:CGRectMake(filterView.frame.size.width-170, 10, 160, 40)];
    ...
    tablDta.frame = CGRectMake(0,linelabel2.frame.size.height+linelabel2.frame.origin.y, self.view.frame.size.width, self.view.frame.size.height-242);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView.tag==4)
    {
        ...
        BookButton.font = [UIFont fontWithName:@"OpenSans-Bold" size:14.0];
...
-(IBAction)leftDateButtonClicked:(id)sender
{
    ...
    label.font=[UIFont fontWithName:@"OpenSans-Bold" size:14.0];
